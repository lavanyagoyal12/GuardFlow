...
class LLMService:
    ...
    def explain_result(self, risk_result: dict[str, Any]) -> dict[str, Any]:
        ...
        if not self.enabled:
            return self._explanation_fallback(
                risk_result,
                "LLM explanation is disabled",
            )

        safe_result = {
            "risk_level": risk_result.get("risk_level"),
            "reasons": list(
                risk_result.get("top_reasons")
                or risk_result.get("evidence")
                or []
            )[:3],
            "recommendation": next(
                iter(risk_result.get("recommendations") or []),
                "Continue normal verification before making a payment",
            ),
        }
        cache_key = self._fingerprint({"task": "explanation", "result": safe_result})
        with self._cache_lock:
            cached = self._cache.get(cache_key)
        if cached is not None:
            return dict(cached)
        ...
        system_prompt = (
            "Write a clear mobile warning for the supplied GuardFlow assessment "
            "in two short sentences and at most 55 words. State why the result "
            "has its supplied level using only the supplied reasons, then give "
            "the supplied recommendation. Do not mention or invent a numeric "
            "score or confidence. Do not output raw rule IDs or add new facts."
        )
        ...
        if raw is None:
            return self._explanation_fallback(
                risk_result,
                fallback_reason or "LLM explanation unavailable",
            )

        result = {
            "available": True,
            "explanation": " ".join(raw.split())[:600],
        }
        with self._cache_lock:
            if len(self._cache) >= self._MAX_CACHE_ENTRIES:
                self._cache.clear()
    ...
    def _explanation_fallback(
        ...
        level = str(risk_result.get("risk_level") or "UNKNOWN").upper()
        reasons = [
        ...
        recommendation = " ".join(str(recommendation).split()).rstrip(". ")

        if level == "LOW":
            if reasons:
                explanation = (
                    "Only limited risk indicators were detected: "
                    f"{'; '.join(reasons)}. {recommendation}."
                )
            else:
                explanation = (
            ...
            explanation = (
                f"Key reasons for this {level.lower()} result: "
                f"{'; '.join(reasons)}. {recommendation}."
            )
        else:
            explanation = f"GuardFlow classified this session as {level.lower()} risk. {recommendation}."
