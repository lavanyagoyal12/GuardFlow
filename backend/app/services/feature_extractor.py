...
class FeatureExtractor:
    ...
    def _extract_content_features(self, signals: dict[str, Any]) -> dict[str, Any]:
        ...
        metadata = self._dict(signals.get("metadata"))
        page_metadata = self._dict(detector.get("page_metadata"))
        ...
        # These bounded snippets are used only for semantic analysis.  The
        # complete PAGE_ANALYSIS remains unchanged in Event.payload; passing a
        # short excerpt avoids sending cookies, the full DOM, link lists, or
        # other unnecessary webpage data to the local model.
        page_title = self._clip_text(
            signals.get("title") or page_metadata.get("title") or metadata.get("og_title"),
            300,
        )
        page_description = self._clip_text(
            metadata.get("description") or page_metadata.get("description"),
            500,
        )
        visible_text_excerpt = self._clip_text(signals.get("visible_text_excerpt"), 1200)
        ...
        return {
            ...
            "registration_fee_samples": self._text_samples(registration_requests, 3, 180),
            "qr_payment_samples": self._text_samples(qr_prompts, 3, 180),
            "government_reference_samples": self._text_samples(government, 3, 180),
            "payment_button_samples": self._text_samples(payment_buttons, 3, 100),
        }
    ...
    @classmethod
    def _clip_text(cls, value: Any, limit: int) -> str:
        # Collapse whitespace so prompt size is predictable and detector
        # evidence cannot smuggle a large block of markup into the request.
        text = " ".join(cls._text(value).split())
        return text[:limit]

    @classmethod
    def _text_samples(cls, values: list[Any], count: int, limit: int) -> list[str]:
        samples = [cls._clip_text(value, limit) for value in values[:count]]
