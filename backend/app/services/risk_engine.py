...
class RiskEngine:
    ...
    # Semantic output can add nuance that detector counts cannot express, but
    # its total contribution is deliberately capped.  The model never supplies
    # points or a final score; these fixed rules remain the scoring authority.
    SEMANTIC_RULES: tuple[Rule, ...] = (
        ("content.semantic_phishing_intent", 12, "Semantic analysis detected phishing intent", lambda c: c["semantic"].get("phishing_intent", False)),
        ("content.semantic_fake_branding", 12, "Semantic analysis detected potentially deceptive branding", lambda c: c["semantic"].get("fake_branding", False)),
        ("content.semantic_social_engineering", 8, "Semantic analysis detected social-engineering language", lambda c: c["semantic"].get("social_engineering", False)),
        ("content.semantic_urgency", 5, "Semantic analysis detected urgency language", lambda c: c["semantic"].get("urgency_language", False)),
    )
    MAX_SEMANTIC_CONTENT_POINTS = 20
    ...
    def calculate(
        ...
        context = {
            ...
            "semantic": website_analysis.get("semantic_analysis") or {},
        }
        ...
        semantic_score, semantic_rules, semantic_evidence = self._apply_rules(
            self.SEMANTIC_RULES,
            context,
        )
        semantic_points = min(self.MAX_SEMANTIC_CONTENT_POINTS, semantic_score)
        content_score = min(100, content_score + semantic_points)
        ...
        # Rank explanation reasons by the component that contributed the most.
        # This is display-only metadata and has no effect on the score.
        reason_groups = sorted(
            (
                (url_score, url_evidence),
                (content_score, content_evidence + semantic_evidence),
                (behaviour_score, behaviour_evidence),
                (transaction_score, transaction_evidence),
                (correlation_bonus, correlation_evidence),
            ),
            key=lambda item: item[0],
            reverse=True,
        )
        top_reasons = self._deduplicate(
            [reason for _, group in reason_groups for reason in group]
        )[:3]
        ...
        return {
            ...
            "semantic_content_points": semantic_points,
            "llm_analysis_available": context["semantic"].get("available") is True,
