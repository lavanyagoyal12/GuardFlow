...
void StateMachine::setState(SystemState newState) {
    ...
    // Default: alarm off in every state. The two states that need it
    // (HIGH_RISK and WAITING_FOR_NFC) turn it back on explicitly below -
    // this guarantees the buzzer can never get stuck on in some other
    // state, even if a future state gets added here.
    _buzzer.stopAlarm();
...
String StateMachine::getLastNfcUid() const {
    return _nfc.getLastUidString();
}
...
void StateMachine::updateAuthorized() {
    unsigned long elapsed = millis() - _stateEnteredAt;
    if (!_checkmarkShown && elapsed >= 1200) {
        _matrix.showCheckmark();
        _checkmarkShown = true;
    }
    if (elapsed >= 4000) {
        setState(SystemState::IDLE);
    }
